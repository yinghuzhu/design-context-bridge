# Design Replicate cross-agent evaluations
...
1. 已安装当前仓库的 `design-context` CLI 和 `design-replicate` Skill；Codex 可用 `$design-replicate`，Claude Code 可用 `/design-replicate`。
...
```bash
EVAL_ROOT="$PWD/evals/design-replicate"
CASE_NAME="new-page"
RUN_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/design-replicate-eval.XXXXXX")"
...
保持 expected report 对 Agent 不可见。`$design-replicate` 使用单引号式文本写入请求，避免 shell 把 `$` 当变量展开：
...
```bash
case "$RUN_ROOT" in
  "${TMPDIR:-/tmp}"/design-replicate-eval.*) rm -rf -- "$RUN_ROOT" ;;
